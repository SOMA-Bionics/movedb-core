...
class OpenSimExporter:
    ...
    def to_trc(
        ...
        if output_units is not None and self.trial.points.units != output_units:
            logger.info(
                f"Output units {output_units} do not match points units {self.trial.points.units}. Converting coordinates."
            )
        ...
        for frame in range(
            self.trial.points.first_frame, self.trial.points.last_frame + 1
        ):
            row = []
    ...
    def export_force_platforms(
        ...
        applied_bodies: dict[
            int, str
        ],  # Expected to be a dict of {platform_index: body_name} where platform_index is 1-based
        ...
        data = np.zeros(
            (len(self.trial.force_platforms) * 9, len(time_col))
        ...
        for i, fp in enumerate(self.trial.force_platforms):
            ...
            if fp.unit_force != unit_force:
                ...
                force_conversion_factor = current_units.convertTo(
                    osim.Units(unit_force)
                )
            if fp.unit_position != unit_position:
                ...
                position_conversion_factor = current_units.convertTo(
                    osim.Units(unit_position)
                )
            if fp.unit_moment != unit_moment:
                ...
                moment_conversion_factor = current_units.convertTo(
                    osim.Units(unit_moment)
                )
        ...
        if "nRows" not in metadata:
            n_frames = self.trial.force_platforms[
                0
        ...
        if "nColumns" not in metadata:
            n_columns = (
                len(self.trial.force_platforms) * 9
            )  # 3 forces, 3 moments, 3 center of pressure
    ...
    def run_inverse_kinematics(
        ...
        ik_tool.setMarkerDataFileName(
            f"{self.trial.name}.trc" if not trc_path else trc_path
        )
