...
def calculate_cop(force: np.ndarray, moment: np.ndarray, z_offset: float) -> tuple[np.ndarray, np.ndarray]:
    ...
    """
    raise DeprecationWarning("This function is deprecated. Use ezc3d force platform filter instead.")
...
def sto_to_df(file_path: str) -> tuple[pl.DataFrame, dict[str,str]]:
    ...
    df = df.with_columns([pl.col(col).cast(pl.String).str.strip_chars().cast(pl.Float64) for col in df.columns])
